...
function escapeCsvField(field) {
    ...
    if (stringField.includes(',') || stringField.includes('\n') || stringField.includes('"')) {
        // Escape existing double quotes by doubling them ("" becomes "")
...
document.getElementById('assessmentForm').addEventListener('submit', function(event) {
    ...
    try {
        ...
        // --- 3. Collect Responses & Validate Completion ---
        console.log("Collecting domain responses...");
        const allResponses = {};
        let missingSelection = false;
        document.querySelectorAll('.question-group').forEach(el => el.style.border = '1px solid #eee');
        ...
        domains.forEach(domain => {
            ...
            questions.forEach(qGroup => {
                const questionId = qGroup.getAttribute('data-question-id');
                 if (!questionId) { // Added check for missing data-question-id
                    console.error("Missing data-question-id attribute on a question group inside fieldset:", domain.name, qGroup);
                    throw new Error(`HTML structure error: Missing data-question-id for a question in ${domain.name}`);
                 }
        ...
        console.log("Scores calculated:", domainScores);
        ...
        /* ... Chart.js code remains commented out ... */

        // --- 7. Prepare Data & Display JSON (Simulated Save) ---
        const assessmentRecord = {
            metadata: { timestamp: new Date().toISOString(), assessmentVersion: "V1.4-CSV" },
            generalInfo: appInfo,
            responses: allResponses, // Note: includes all raw responses
            scores: domainScores
        };
        console.log("Assessment Record (Simulated Save - JSON):", JSON.stringify(assessmentRecord, null, 2));
        ...
        // --- 7b. Generate and Display CSV Data ---
        console.log("Generating CSV data...");
        try {
            const csvHeaders = [
                "Timestamp", "AssessmentVersion", "AppName", "VersionNumber",
                "UpdateDate", "DeveloperName", "DeveloperEmail", "DeveloperCountry",
                "DeveloperTelephone", "EnglishAccess", "AppAvailability", "MajorFocus",
                "IntendedUsers",
                // Scores
                "AcceptabilityScore", "SafetyScore", "UsabilityScore", "PrivacyScore", "TechnicalScore"
            ];

            const scoreAcceptability = typeof assessmentRecord.scores.acceptability === 'number' ? assessmentRecord.scores.acceptability.toFixed(1) + '%' : 'N/A';
            const scoreSafety = typeof assessmentRecord.scores.safety === 'number' ? assessmentRecord.scores.safety.toFixed(1) + '%' : 'N/A';
            const scoreUsability = typeof assessmentRecord.scores.usability === 'number' ? assessmentRecord.scores.usability.toFixed(1) + '%' : 'N/A';
            const scorePrivacy = typeof assessmentRecord.scores.privacy === 'number' ? assessmentRecord.scores.privacy.toFixed(1) + '%' : 'N/A';
            const scoreTechnical = typeof assessmentRecord.scores.technical === 'number' ? assessmentRecord.scores.technical.toFixed(1) + '%' : 'N/A';

            const availabilityString = assessmentRecord.generalInfo.appAvailability.join('; ');

            const csvDataRow = [
                assessmentRecord.metadata.timestamp,
                assessmentRecord.metadata.assessmentVersion,
                assessmentRecord.generalInfo.appName,
                assessmentRecord.generalInfo.versionNumber,
                assessmentRecord.generalInfo.updateDate,
                assessmentRecord.generalInfo.developerName,
                assessmentRecord.generalInfo.developerEmail,
                assessmentRecord.generalInfo.developerCountry,
                assessmentRecord.generalInfo.developerTelephone,
                assessmentRecord.generalInfo.q0_4_englishAccess,
                availabilityString,
                assessmentRecord.generalInfo.majorFocus,
                assessmentRecord.generalInfo.intendedUsers,
                scoreAcceptability,
                scoreSafety,
                scoreUsability,
                scorePrivacy,
            ...
            const escapedHeaders = csvHeaders.map(escapeCsvField).join(',');
            const escapedDataRow = csvDataRow.map(escapeCsvField).join(',');
            const csvString = `${escapedHeaders}\n${escapedDataRow}`;

            const csvTextArea = document.getElementById('csvDataOutput');
            if (csvTextArea) {
                csvTextArea.value = csvString;
                console.log("CSV data generated and displayed.");
            } else {
                console.error("HTML structure error: Cannot find csvDataOutput textarea");
            }
        } catch(csvError) {
             console.error("Error generating CSV data:", csvError);
             const csvTextArea = document.getElementById('csvDataOutput');
             if (csvTextArea) { csvTextArea.value = "Error generating CSV data. See console for details."; }
        }
